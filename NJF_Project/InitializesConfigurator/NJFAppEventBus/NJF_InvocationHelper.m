...
@implementation NJF_InvocationHelper
...
+ (id _Nullable)performClass:(NSString *)className selector:(nonnull SEL)selector params:(NSDictionary *_Nullable)params {
    ...
    if (class != nil) {
        ...
        return [NSError errorWithDomain:TLInvocationDomain code:NJF_InvocationError_No_Class userInfo:nil];
    }
}
...
return @(val);                    \
    ...
    if (![target respondsToSelector:selector] || (methodSig == nil)) {
        ...
        return [NSError errorWithDomain:TLInvocationDomain code:NJF_InvocationError_No_Selector userInfo:nil];
    }
    
    if (ACTION_PARAMS_COUNT(action) != (params == nil ? 0:1)) {
        NSLog(@"参数个数与方法不匹配");
        return [NSError errorWithDomain:TLInvocationDomain code:NJF_InvocationError_Param_Unmatch userInfo:nil];
    }
